var ks="lift";var pt=["exercises","workouts","sets","stateOfMind","medications","doseEvents"],$e=null;function j(){return $e?Promise.resolve($e):new Promise((e,t)=>{let s=indexedDB.open(ks,4);s.onerror=()=>t(s.error),s.onsuccess=()=>{$e=s.result,e($e)},s.onupgradeneeded=()=>{let o=s.result;if(!o.objectStoreNames.contains("exercises")){let n=o.createObjectStore("exercises",{keyPath:"id"});n.createIndex("name","name",{unique:!1}),n.createIndex("category","category",{unique:!1})}if(o.objectStoreNames.contains("workouts")||o.createObjectStore("workouts",{keyPath:"id"}).createIndex("startedAt","startedAt",{unique:!1}),!o.objectStoreNames.contains("sets")){let n=o.createObjectStore("sets",{keyPath:"id"});n.createIndex("workoutId","workoutId",{unique:!1}),n.createIndex("exerciseId","exerciseId",{unique:!1})}if(o.objectStoreNames.contains("stateOfMind")||o.createObjectStore("stateOfMind",{keyPath:"id"}).createIndex("date","date",{unique:!1}),o.objectStoreNames.contains("medications")||o.createObjectStore("medications",{keyPath:"id"}),!o.objectStoreNames.contains("doseEvents")){let n=o.createObjectStore("doseEvents",{keyPath:"id"});n.createIndex("medicationId","medicationId",{unique:!1}),n.createIndex("date","date",{unique:!1})}}})}function de(e){return new Promise((t,s)=>{e.onsuccess=()=>t(e.result),e.onerror=()=>s(e.error)})}async function ue(e,t="readonly"){return(await j()).transaction(e,t).objectStore(e)}function ee(e,t,s){return new Promise((o,n)=>{let i=e.transaction(t,"readwrite"),r;try{r=s(i)}catch(a){try{i.abort()}catch{}n(a);return}i.oncomplete=()=>o(r),i.onerror=()=>n(i.error),i.onabort=()=>n(i.error)})}async function B(e){return de((await ue(e)).getAll())}async function pe(e,t){return de((await ue(e)).get(t))}async function q(e,t){return await de((await ue(e,"readwrite")).put(t)),t}async function fe(e,t){let s=await j();return ee(s,e,o=>{let n=o.objectStore(e);for(let i of t)n.put(i)})}async function ie(e,t){return de((await ue(e,"readwrite")).delete(t))}async function Ve(e,t){if(t.length===0)return;let s=await j();return ee(s,e,o=>{let n=o.objectStore(e);for(let i of t)n.delete(i)})}async function Me(e,t,s){let o=await ue(e);return de(o.index(t).getAll(s))}async function ft(e){let t=await j();return ee(t,pt,s=>{for(let o of pt){let n=s.objectStore(o);n.clear();for(let i of e[o]??[])n.put(i)}})}function Q(e){let t=new Set;for(let s of e)s.completed&&t.add(s.workoutId);return e.filter(s=>s.completed||!t.has(s.workoutId))}async function re(){return(await B("workouts")).find(t=>!t.endedAt)??null}async function X(){return(await B("workouts")).filter(t=>t.endedAt).sort((t,s)=>s.startedAt-t.startedAt)}async function mt(e){return(await Me("sets","workoutId",e)).sort((s,o)=>s.order-o.order)}async function xs(e){return await Me("sets","exerciseId",e)}async function vt(e,t=null){let s=await xs(e),o=new Map;for(let r of s)t&&r.workoutId===t||(o.has(r.workoutId)||o.set(r.workoutId,[]),o.get(r.workoutId).push(r));if(o.size===0)return[];let i=(await Promise.all(Array.from(o.keys()).map(r=>pe("workouts",r)))).filter(Boolean).sort((r,a)=>(a.startedAt??0)-(r.startedAt??0));return i.length===0?[]:o.get(i[0].id).sort((r,a)=>r.order-a.order)}function ht(e,t,s=null){let o=new Map(t.map(r=>[r.id,r.startedAt??0])),n=new Map;for(let r of e){if(r.workoutId===s||!o.has(r.workoutId)||(r.weight||0)<=0||(r.reps||0)<=0)continue;let a=n.get(r.exerciseId);a||n.set(r.exerciseId,a=new Map);let l=a.get(r.workoutId);l||a.set(r.workoutId,l=[]),l.push(r)}let i=new Map;for(let[r,a]of n){let l=[...a.keys()].sort((b,S)=>o.get(S)-o.get(b)),u=new Map;for(let b of l){let S=a.get(b).sort((E,v)=>E.order-v.order),d=S.every(E=>E.setType==null),f=0,w=0;S.forEach((E,v)=>{if(d){let x=`any#${v+1}`;u.has(x)||u.set(x,E);return}let m=E.setType||"working",$=m==="warmup"?w+=1:f+=1,k=`${m}#${$}`;u.has(k)||u.set(k,E)})}i.set(r,u)}return i}var Ss={barbell:"Barbell",dumbbell:"Dumbbell",machine:"Machine",cable:"Cable",bodyweight:"Bodyweight",kettlebell:"Kettlebell",band:"Bands",bands:"Bands"},$s=/\s*\((barbell|dumbbell|machine|cable|bodyweight|kettlebell|bands?)\)$|\s+(Machine|Barbell|Dumbbell|Cable|Kettlebell)$/i;async function Ms(e,t){let s=await j(),o=await Me("sets","exerciseId",e);return ee(s,["sets","exercises"],n=>{let i=n.objectStore("sets");for(let r of o)i.put({...r,exerciseId:t});return n.objectStore("exercises").delete(e),o.length})}async function yt(){let e=await B("exercises"),t=e.filter(i=>/butterfly/i.test(i.name||""));if(t.length===0)return 0;let s=e.filter(i=>/chest fly/i.test(i.name||"")&&!t.some(r=>r.id===i.id)),o=s.find(i=>(i.equipment||"")==="Machine")||s[0],n=0;for(let i of t)o?n+=await Ms(i.id,o.id):await q("exercises",{...i,name:"Chest Fly",equipment:"Machine"});return n}async function gt(){let e=await B("exercises"),t=[];for(let s of e){let o=(s.name||"").match($s);if(!o)continue;let n=s.name.slice(0,o.index).trim();if(!n||/smith$/i.test(n))continue;let i=(o[1]||o[2]).toLowerCase();t.push({...s,name:n,equipment:Ss[i]||s.equipment})}return t.length>0&&await fe("exercises",t),t.length}async function wt(){let[e,t,s]=await Promise.all([B("exercises"),B("sets"),B("workouts")]),o=new Set(e.filter(u=>u.category==="Cardio").map(u=>u.id));if(o.size===0)return{exercises:0,sets:0,workouts:0};let n=t.filter(u=>o.has(u.exerciseId)),i=new Map;for(let u of t)o.has(u.exerciseId)||i.set(u.workoutId,(i.get(u.workoutId)||0)+1);let r=new Set(n.map(u=>u.workoutId)),a=s.filter(u=>r.has(u.id)&&!i.get(u.id)),l=await j();return await ee(l,["exercises","sets","workouts"],u=>{let b=u.objectStore("exercises"),S=u.objectStore("sets"),d=u.objectStore("workouts");for(let f of o)b.delete(f);for(let f of n)S.delete(f.id);for(let f of a)d.delete(f.id)}),{exercises:o.size,sets:n.length,workouts:a.length}}async function bt(e){let[t,s,o]=await Promise.all([B("exercises"),B("sets"),B("workouts")]),n=t.filter(d=>d.category==="Other");if(n.length===0)return{recategorized:0,deleted:0,workouts:0};let i=[],r=new Set;for(let d of n){let f=e(d.name);f==="Cardio"?r.add(d.id):i.push({...d,category:f&&f!=="Other"?f:"Full Body"})}let a=s.filter(d=>r.has(d.exerciseId)),l=new Map;for(let d of s)r.has(d.exerciseId)||l.set(d.workoutId,(l.get(d.workoutId)||0)+1);let u=new Set(a.map(d=>d.workoutId)),b=o.filter(d=>u.has(d.id)&&!l.get(d.id)),S=await j();return await ee(S,["exercises","sets","workouts"],d=>{let f=d.objectStore("exercises"),w=d.objectStore("sets"),E=d.objectStore("workouts");for(let v of i)f.put(v);for(let v of r)f.delete(v);for(let v of a)w.delete(v.id);for(let v of b)E.delete(v.id)}),{recategorized:i.length,deleted:r.size,workouts:b.length}}async function Ee(e){let t=await j(),s=await Me("sets","workoutId",e);return ee(t,["workouts","sets"],o=>{o.objectStore("workouts").delete(e);let n=o.objectStore("sets");for(let i of s)n.delete(i.id)})}var F=()=>crypto&&crypto.randomUUID?crypto.randomUUID():"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,e=>{let t=Math.random()*16|0;return(e==="x"?t:t&3|8).toString(16)});function me(e){return e==null?"0":Math.abs(e-Math.round(e))<.001?String(Math.round(e)):e.toFixed(1)}function ve(e){return`${me(e)} lbs`}function kt(e){let t=Math.max(0,Math.floor(e)),s=Math.floor(t/3600),o=Math.floor(t%3600/60),n=t%60;return s>0?`${s}:${String(o).padStart(2,"0")}:${String(n).padStart(2,"0")}`:`${o}:${String(n).padStart(2,"0")}`}function _e(e){let t=Math.max(0,Math.floor(e)),s=Math.floor(t/3600),o=Math.floor(t%3600/60);return s>0?`${s}h ${o}m`:`${o}m`}function Y(e){return Math.round(e).toLocaleString()}function ae(e){return`${Y(e)} lbs`}function z(e){return(e instanceof Date?e:new Date(e)).toLocaleDateString(void 0,{month:"numeric",day:"numeric",year:"2-digit"})}function xt(e){return(e instanceof Date?e:new Date(e)).toLocaleDateString(void 0,{weekday:"short",month:"short",day:"numeric",year:"numeric"})}function Ye(e,t=200){let s=null;return(...o)=>{clearTimeout(s),s=setTimeout(()=>e(...o),t)}}function M(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function I(e,t=1800,s={}){let o=document.querySelector(".toast");o&&o.remove();let n=document.createElement("div");n.className="toast",n.textContent=e,s.persistUntilClick?(n.classList.add("toast-clickable"),n.addEventListener("click",()=>n.remove())):setTimeout(()=>n.remove(),t),document.body.appendChild(n)}var Ue=new EventTarget;function H(e,t){Ue.dispatchEvent(new CustomEvent(e,{detail:t}))}function Ke(e,t){return Ue.addEventListener(e,t),()=>Ue.removeEventListener(e,t)}function W({html:e,onMount:t}){let s=document.createElement("div");s.className="sheet-backdrop",s.innerHTML='<div class="sheet"></div>';let o=s.querySelector(".sheet");o.innerHTML=e;let n=Es();document.body.appendChild(s);function i(){let l=window.visualViewport;if(!l){o.style.maxHeight=`${window.innerHeight-n-10}px`;return}let u=Math.max(window.innerHeight,document.documentElement.clientHeight),b=Math.max(0,u-l.height-l.offsetTop);b>0?(o.style.paddingBottom=`${b}px`,o.style.maxHeight=`${l.height-n-10+b}px`):(o.style.paddingBottom="",o.style.maxHeight=`${l.height-n-10}px`)}i();let r=window.visualViewport;r?.addEventListener("resize",i),r?.addEventListener("scroll",i);function a(){s.remove(),r?.removeEventListener("resize",i),r?.removeEventListener("scroll",i)}return s.dismissSheet=a,s.addEventListener("click",l=>{l.target===s&&a()}),t?.(o,a),a}function Es(){let e=document.createElement("div");e.style.cssText="position:fixed;top:0;left:0;width:0;height:env(safe-area-inset-top);",document.body.appendChild(e);let t=e.offsetHeight||0;return e.remove(),t}function Le(){return'<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor" style="color: var(--red);"><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"/></svg>'}function St(){return'<svg viewBox="0 0 24 24" width="22" height="22" fill="currentColor"><path d="M16 5l-1.42 1.42-1.59-1.59V16h-2V4.83L9.41 6.42 8 5l4-4 4 4zm4 5v11c0 1.1-.9 2-2 2H6c-1.11 0-2-.9-2-2V10c0-1.11.89-2 2-2h3v2H6v11h12V10h-3V8h3c1.1 0 2 .89 2 2z"/></svg>'}function J(e){return`<div class="empty-state"><div class="empty-icon">!</div><h2>Couldn't load</h2><p>${M(e.message||String(e))}</p></div>`}var he=["Pectorals","Anterior Deltoid","Lateral Deltoid","Posterior Deltoid","Triceps","Biceps","Forearms","Lats","Upper Back","Lower Back","Traps","Quadriceps","Hamstrings","Glutes","Adductors","Abductors","Calves","Abs","Obliques"];function Ls(e){let t=new Map(he.map((s,o)=>[s,o]));return[...e].sort((s,o)=>(t.get(s)??999)-(t.get(o)??999)||s.localeCompare(o))}var De=["Barbell","Dumbbell","Machine","Cable","Bodyweight","Other"];function K(e){let t=e?.equipment,s=e?.name??"";return t&&t!=="Other"?`${s} (${t})`:s}function te(e){let t=e?[e.equipment,R(e)].filter(Boolean).join(" \xB7 "):"";return`
  ...
  `;let S=e.querySelector('[data-role="scrub"]'),d=e.querySelector('[data-role="chart"]'),f=e.querySelector('[data-role="range"]'),w=e.querySelector(".chart-range"),E=[...e.querySelectorAll(".chart-slider-ticks span")],v=s.unit||"lbs",m=null;function $(){let p=u(),c=Vs(p,n,v);d.innerHTML=c.html,m=c.geom;let h=p.flat();if(h.length>=2){let y=Math.min(...h.map(D=>D.date)),g=Math.max(...h.map(D=>D.date));f.innerHTML=`<span>${ot(y)}</span><span>${ot(g)}</span>`}else f.innerHTML="";E.forEach((y,g)=>y.classList.toggle("active",g===r))}w.addEventListener("input",()=>{r=Number(w.value),T(),$()});let k=[...e.querySelectorAll(".chart-legend .legend-item")];for(let p of k)p.addEventListener("click",()=>{let c=Number(p.dataset.i);l=l===c?null:c,k.forEach((h,y)=>{h.classList.toggle("dimmed",l!==null&&y!==l),h.setAttribute("aria-pressed",String(l===y))}),T(),$()});function x(p){if(!m||m.pts.length<2)return;let c=d.querySelector("svg"),h=c?.getScreenCTM();if(!h)return;let y=new DOMPoint(p,0).matrixTransform(h.inverse()).x,g=0,D=1/0;m.pts.forEach((O,U)=>{let _=Math.abs(O.x-y);_<D&&(D=_,g=U)});let L=m.pts[g],C=c.querySelector(".chart-scrub-line"),P=c.querySelector(".chart-scrub-dot");C&&(C.setAttribute("x1",L.x),C.setAttribute("x2",L.x),C.removeAttribute("visibility")),P&&(P.setAttribute("cx",L.x),P.setAttribute("cy",L.y),P.style.fill=L.color,P.removeAttribute("visibility"));let V=L.label?` \xB7 ${L.label}`:"";S.textContent=`${ot(L.date)}${V} \xB7 ${Math.round(L.value).toLocaleString()} ${v}`}function T(){S.textContent="";let p=d.querySelector("svg");p?.querySelector(".chart-scrub-line")?.setAttribute("visibility","hidden"),p?.querySelector(".chart-scrub-dot")?.setAttribute("visibility","hidden")}let A=!1;d.addEventListener("pointerdown",p=>{A=!0,d.setPointerCapture?.(p.pointerId),x(p.clientX)}),d.addEventListener("pointermove",p=>{A&&x(p.clientX)});for(let p of["pointerup","pointercancel"])d.addEventListener(p,()=>{A=!1,T()});$()}function ot(e){return new Date(e).toLocaleDateString(void 0,{month:"short",day:"numeric",year:"numeric"})}function Vs(e,t,s){let i={top:16,right:14,bottom:14,left:52},r=400-i.left-i.right,a=200-i.top-i.bottom,l=e.flat();if(l.length===0)return{html:`<svg viewBox="0 0 400 200"><text x="${400/2}" y="${200/2}" text-anchor="middle" class="chart-axis-label">No data in range</text></svg>`,geom:null};if(l.length===1){let g=l[0],D=t[e.findIndex(P=>P.length>0)]?.color||"var(--accent)",L=i.left+r/2,C=i.top+a/2;return{html:`<svg viewBox="0 0 400 200"><circle cx="${L}" cy="${C}" r="4" class="chart-point" style="fill: ${D};"/><text x="${L}" y="${C-10}" text-anchor="middle" class="chart-axis-label">${Math.round(g.value).toLocaleString()} ${s}</text></svg>`,geom:null}}let u=l.map(g=>g.date),b=l.map(g=>g.value),S=Math.min(...u),d=Math.max(...u),f=Math.max(...b),w=Math.min(...b),E=Math.max(f-w,1),v=Math.max(0,w-E*.12),m=f+E*.12,$=g=>i.left+(g-S)/Math.max(d-S,1)*r,k=g=>i.top+a-(g-v)/(m-v)*a,x=4,T=g=>Math.round(g).toLocaleString(),A=Array.from({length:x+1},(g,D)=>{let L=v+(m-v)*D/x,C=k(L);return`<text x="${i.left-6}" y="${C+3}" text-anchor="end" class="chart-axis-label">${T(L)}</text>`}).join(""),p=Array.from({length:x+1},(g,D)=>{let L=i.top+a*D/x;return`<line x1="${i.left}" x2="${400-i.right}" y1="${L}" y2="${L}" class="chart-axis-line"/>`}).join(""),c=[],h=e.map((g,D)=>{let L=t[D],C=g.map(P=>({x:$(P.date),y:k(P.value)}));return g.forEach((P,V)=>c.push({...C[V],date:P.date,value:P.value,label:L.label,color:L.color})),C.length===0?"":C.length===1?`<circle cx="${C[0].x}" cy="${C[0].y}" r="3.5" class="chart-point" style="fill: ${L.color};"/>`:`<path d="${Us(C)}" class="chart-line" style="stroke: ${L.color};"/>`}).join("");return{html:`
  ...
  `,geom:{pts:c}}}function Us(e){if(e.length<2)return"";let t=`M ${e[0].x.toFixed(1)} ${e[0].y.toFixed(1)}`;for(let s=0;s<e.length-1;s++){let o=e[s===0?0:s-1],n=e[s],i=e[s+1],r=e[s+2]||i,a=n.x+(i.x-o.x)/6,l=n.y+(i.y-o.y)/6,u=i.x-(r.x-n.x)/6,b=i.y-(r.y-n.y)/6;t+=` C ${a.toFixed(1)} ${l.toFixed(1)}, ${u.toFixed(1)} ${b.toFixed(1)}, ${i.x.toFixed(1)} ${i.y.toFixed(1)}`}return t}var Z=null;function Nt(e){let t=!0;return jt().then(s=>{t&&(Z=s,Pe(e))}).catch(s=>{t&&(e.container.innerHTML=J(s))}),()=>{t=!1}}async function jt(){let[e,t,s]=await Promise.all([X(),B("sets"),B("exercises")]),o=new Map(s.map(w=>[w.id,w])),n=new Map;for(let w of Q(t))n.has(w.workoutId)||n.set(w.workoutId,[]),n.get(w.workoutId).push(w);let i=0,r=0,a=new Map,l=new Map,u=new Map,b=Bt(e,n,o);for(let w of e){let E=n.get(w.id)||[],v=E.reduce((m,$)=>m+$.weight*$.reps,0);if(i+=v,r+=E.length,v>0){let m=b.get(w.id);a.has(m)||a.set(m,[]),a.get(m).push({date:w.startedAt,value:v})}for(let m of E){let $=o.get(m.exerciseId);if(!$)continue;let k=l.get(m.exerciseId)||{id:m.exerciseId,exercise:$,count:0};if(k.count+=1,l.set(m.exerciseId,k),m.weight>0&&m.reps>0){let x=u.get(m.exerciseId);(!x||m.weight>x.weight||m.weight===x.weight&&m.reps>x.reps)&&u.set(m.exerciseId,{id:m.exerciseId,weight:m.weight,reps:m.reps,date:w.startedAt,name:K($)})}}}let S=Array.from(l.entries()).sort((w,E)=>E[1].count-w[1].count).map(([,w])=>w),d=Array.from(u.values()).sort((w,E)=>E.weight-w.weight),f=N.filter(w=>a.has(w)).map(w=>({label:Te[w].short,color:Ae(w),points:a.get(w)}));return{workouts:e,allSets:t,allExercises:s,exMap:o,setsByWorkout:n,totalVolume:i,totalSets:r,volumeSeries:f,topExercises:S,prs:d}}function Pe(e){if(e.setTitle("Progress"),e.setBack(null),e.setAction({label:"Backup and restore",html:St(),onClick:()=>Ft()}),e.container.scrollTop=0,!Z||Z.workouts.length===0){e.container.innerHTML=`
  ...
  `;let a=e.container.querySelector(".volume-chart-mount");a&&n.length>0&&ye(a,n,{unit:"lbs"});for(let l of e.container.querySelectorAll("[data-page]"))l.addEventListener("click",()=>{let u=l.dataset.page;u==="trained"?_s(e):u==="prs"?Ys(e):u==="history"&&zt(e)})}function _s(e){e.setTitle("Most-Trained"),e.setBack(()=>Pe(e)),e.setAction(null);let{topExercises:t}=Z;e.container.innerHTML=`
    <div class="list" style="margin-top: 16px;">
      ${t.map(s=>`
  ...
  `,e.container.scrollTop=0,nt(e)}function Ys(e){e.setTitle("Personal Records"),e.setBack(()=>Pe(e)),e.setAction(null);let{prs:t}=Z;e.container.innerHTML=`
  ...
  `,e.container.scrollTop=0,nt(e)}function nt(e){for(let t of e.container.querySelectorAll("[data-exercise-id]"))t.addEventListener("click",()=>{Oe(t.dataset.exerciseId)})}function zt(e){e.setTitle("Workout History"),e.setBack(()=>Pe(e)),e.setAction(null);let{workouts:t,setsByWorkout:s,exMap:o}=Z;e.container.innerHTML=`
    <div class="list" style="margin-top: 16px;">
      ${t.map(n=>Ks(n,s.get(n.id)||[],o)).join("")}
    </div>
  `,e.container.scrollTop=0;for(let n of e.container.querySelectorAll("[data-workout-id]"))n.addEventListener("click",()=>{let i=n.dataset.workoutId;Gs(e,i).catch(r=>{e.container.innerHTML=J(r)})})}function Ks(e,t,s){let o=t,n=o.reduce((l,u)=>l+u.weight*u.reps,0),i=(e.endedAt-e.startedAt)/1e3,r=[],a=new Set;for(let l of t){if(a.has(l.exerciseId))continue;a.add(l.exerciseId);let u=s.get(l.exerciseId);if(u&&r.push(u.name),r.length>=3)break}return`
  ...
  `}async function Vt(e){let[t,s,o]=await Promise.all([pe("workouts",e),B("exercises"),mt(e)]);if(!t)return null;let n=new Map(s.map(d=>[d.id,d])),i=new Map,r=[];for(let d of o)i.has(d.exerciseId)||(i.set(d.exerciseId,[]),r.push(d.exerciseId)),i.get(d.exerciseId).push(d);let a=Q(o),l=a.reduce((d,f)=>d+f.weight*f.reps,0),u=a.length,b=(t.endedAt-t.startedAt)/1e3,S=`
    ...
    ${r.map(d=>{let f=n.get(d),w=i.get(d),E=0,v=0;return`
        ${f?`<button class="section section-link" data-exercise-id="${M(d)}">${M(K(f))}<span class="name-chevron">\u203A</span></button>`:'<div class="section">Unknown exercise</div>'}
        <div class="form-section">
          ${w.map($=>{let x=($.setType||"working")==="warmup"?`W${++v}`:String(++E);return`
              <div class="stat-row">
      ...
      `}).join("")}
  `;return{workout:t,html:S,sets:o}}function Ut(e,t){for(let s of e.querySelectorAll("input.hist-input[data-set-id]"))s.addEventListener("input",async()=>{let o=t.find(n=>n.id===s.dataset.setId);o&&(s.dataset.field==="weight"?o.weight=parseFloat(s.value)||0:o.reps=parseInt(s.value,10)||0,await q("sets",{...o}))})}async function Gs(e,t){e.setBack(async()=>{Z=await jt(),zt(e)}),e.setAction({label:"Delete workout",html:Le(),onClick:async()=>{confirm("Delete this workout?")&&(await Ee(t),H("data:changed"))}});let s=await Vt(t);if(!s){e.container.innerHTML=J({message:"Workout not found."});return}e.setTitle(s.workout.name),e.container.innerHTML=s.html,e.container.scrollTop=0,nt(e),Ut(e.container,s.sets)}async function _t(e){let t=await Vt(e);if(!t)return;let s=W({html:`
      <div class="sheet-header">
    ...
    ${r.length>0?`
      ...
      <div class="form-section">
        ${r.slice(-30).reverse().map(d=>`
          <button class="stat-row recent-set" data-workout-id="${M(d.workoutId)}">
  ...
  `;return{exercise:t,completed:r,chartData:b,html:S}}var Zt=["Amazed","Excited","Happy","Joyful","Content","Calm","Grateful","Hopeful","Confident","Proud","Surprised","Indifferent","Anxious","Stressed","Overwhelmed","Frustrated","Angry","Irritated","Sad","Lonely","Discouraged","Drained","Worried","Embarrassed"],es=["Health","Fitness","Self-Care","Hobbies","Identity","Community","Family","Friends","Partner","Work","Education","Money","Weather","Tasks"],it=[["taken","Taken"],["skipped","Skipped"],["snoozed","Snoozed"],["notInteracted","Not interacted"]],Xs=new Set(["taken","skipped","snoozed","notInteracted"]);function Js(e){let t=Number(e);return isFinite(t)?Math.max(-1,Math.min(1,t)):0}async function ts({kind:e,valence:t,labels:s,associations:o,date:n}){let i={id:F(),kind:e==="dailyMood"?"dailyMood":"momentaryEmotion",date:n||Date.now(),valence:Js(t),labels:s||[],associations:o||[]};return await q("stateOfMind",i),i}async function ss({nickname:e,form:t,hasSchedule:s}){let o=(e||"").trim()||"Medication",n={id:F(),nickname:o,isArchived:!1,hasSchedule:!!s,concept:{identifier:"",displayText:o,form:(t||"").trim(),rxnorm:[]}};return await q("medications",n),n}async function rt({medicationId:e,status:t,date:s,doseQuantity:o}){let n={id:F(),medicationId:String(e),status:Xs.has(t)?t:"taken",date:s||Date.now(),scheduledQuantity:0,doseQuantity:Number(o)||0};return await q("doseEvents",n),n}async function os(e,t){await ie(e,t)}async function at(){let[e,t,s]=await Promise.all([B("stateOfMind"),B("medications"),B("doseEvents")]);return e.sort((o,n)=>o.date-n.date),s.sort((o,n)=>o.date-n.date),{stateOfMind:e,medications:t,doseEvents:s}}var Xt=e=>{let t=new Date(e);return`${t.getFullYear()}-${t.getMonth()}-${t.getDate()}`},Jt=e=>e.length?e.reduce((t,s)=>t+s,0)/e.length:null;function ns(e,t){let s=new Set(t.map(a=>Xt(a.startedAt))),o=[],n=[];for(let a of e)(s.has(Xt(a.date))?o:n).push(a.valence);let i=Jt(o),r=Jt(n);return{onWorkout:i,offWorkout:r,delta:i!=null&&r!=null?i-r:null,onCount:o.length,offCount:n.length}}function is(e,t){let s=new Map;for(let o of t){if(o.status!=="taken"&&o.status!=="skipped")continue;let n=s.get(o.medicationId)??{taken:0,total:0};n.total+=1,o.status==="taken"&&(n.taken+=1),s.set(o.medicationId,n)}return e.map(o=>{let n=s.get(o.id)??{taken:0,total:0};return{medication:o,taken:n.taken,total:n.total,pct:n.total?n.taken/n.total:null}})}var Zs=Object.fromEntries(it),as=e=>new Date(e).toLocaleTimeString(void 0,{hour:"numeric",minute:"2-digit"});async function ct(e,t){e.setTitle("State of Mind"),e.setBack(t),e.setAction(null);let[{stateOfMind:s},o]=await Promise.all([at(),X()]),n=()=>ct(e,t),i=ns(s,o);e.container.innerHTML=`
    ...
    ${s.length?`
      <div class="section">Summary</div>
      <div class="form-section">
        <div class="stat-row"><div class="stat-label">Entries</div><div class="stat-value">${s.length.toLocaleString()}</div></div>
        <div class="stat-row"><div class="stat-label">Range</div><div class="stat-value">${z(s[0].date)} \u2013 ${z(s[s.length-1].date)}</div></div>
        <div class="stat-row"><div class="stat-label">Average mood</div><div class="stat-value">${He(oo(s))}</div></div>
      </div>

      <div class="section">Mood vs. training</div>
      <div class="form-section">
        <div class="stat-row"><div class="stat-label">On workout days</div><div class="stat-value">${i.onWorkout!=null?He(i.onWorkout)+` <span style="color:var(--text-tertiary)">(${i.onCount})</span>`:"\u2014"}</div></div>
        <div class="stat-row"><div class="stat-label">On rest days</div><div class="stat-value">${i.offWorkout!=null?He(i.offWorkout)+` <span style="color:var(--text-tertiary)">(${i.offCount})</span>`:"\u2014"}</div></div>
      ...
      <div class="list">${s.slice(-30).reverse().map(eo).join("")}</div>
    `:cs("\u{1F9E0}","No entries yet. Tap <b>Log State of Mind</b> to start.")}
  `,e.container.scrollTop=0,e.container.querySelector("#som-log").addEventListener("click",()=>io(n)),ds(e,n)}function eo(e){let t=e.labels.length?e.labels.join(", "):e.kind==="dailyMood"?"Daily mood":"Momentary";return`
    <div class="list-row">
      <div class="row-main">
        <div class="row-title">${M(t)}</div>
        <div class="row-subtitle">${z(e.date)} \xB7 ${as(e.date)}${e.associations.length?" \xB7 "+M(e.associations.join(", ")):""}</div>
      </div>
      <div class="row-trailing">${He(e.valence)}</div>
      ${ls("stateOfMind",e.id)}
    </div>`}async function lt(e,t){e.setTitle("Medications"),e.setBack(t),e.setAction(null);let{medications:s,doseEvents:o}=await at(),n=()=>lt(e,t),i=is(s,o),r=new Map(s.map(l=>[l.id,l.nickname||l.concept.displayText])),a=o.slice(-20).reverse();e.container.innerHTML=`
    <div class="action-section">
    ...
    ${s.length?`
      ...
      ${a.length?`
        ...
        <div class="list">${a.map(l=>so(l,r)).join("")}</div>
      `:""}
    `:cs("\u{1F48A}","No medications yet. Tap <b>Add Medication</b>, then log each dose as you take it.")}
  `,e.container.scrollTop=0,e.container.querySelector("#med-add").addEventListener("click",()=>ro(n));for(let l of e.container.querySelectorAll("[data-take]"))l.addEventListener("click",async()=>{await rt({medicationId:l.dataset.take,status:l.dataset.status,date:Date.now(),doseQuantity:1}),I(l.dataset.status==="taken"?"Logged as taken":"Logged as skipped"),n()});for(let l of e.container.querySelectorAll("[data-logat]"))l.addEventListener("click",()=>ao(s,n,l.dataset.logat));ds(e,n)}function to(e){let t=e.medication,s=[t.concept.form||"No form set",e.pct!=null?`${Math.round(e.pct*100)}% taken (${e.taken}/${e.total})`:"no doses yet"].join(" \xB7 ");return`
    <div class="exercise-section">
      <div class="exercise-section-header">
        <div class="row-main">
          <div class="row-title" style="font-weight:600">${M(t.nickname||t.concept.displayText)}</div>
          <div class="row-subtitle">${M(s)}</div>
        </div>
        <button class="menu" data-del-store="medications" data-del-id="${M(t.id)}" aria-label="Delete">\u2715</button>
      </div>
      <div class="med-actions">
        <button class="btn-secondary" data-take="${M(t.id)}" data-status="taken">Taken now</button>
        <button class="btn-secondary" data-take="${M(t.id)}" data-status="skipped">Skip</button>
        <button class="btn-secondary" data-logat="${M(t.id)}">Log at time\u2026</button>
      </div>
    </div>`}function so(e,t){return`
    <div class="list-row">
      <div class="row-main">
        <div class="row-title">${M(t.get(e.medicationId)||"Medication")}</div>
        <div class="row-subtitle">${z(e.date)} \xB7 ${as(e.date)}</div>
      </div>
      <div class="row-trailing">${M(Zs[e.status]||e.status)}</div>
      ${ls("doseEvents",e.id)}
    </div>`}function cs(e,t){return`
    <div class="empty-state" style="padding: 40px 24px; min-height: auto;">
      <div class="empty-icon">${e}</div>
      <p style="color: var(--text-secondary); max-width: 300px;">${t}</p>
    </div>`}function ls(e,t){return`<button class="hz-del" data-del-store="${e}" data-del-id="${M(t)}" aria-label="Delete">\u2715</button>`}function ds(e,t){for(let s of e.container.querySelectorAll("[data-del-id]"))s.addEventListener("click",async()=>{confirm("Delete this entry?")&&(await os(s.dataset.delStore,s.dataset.delId),t())})}function oo(e){return e.reduce((t,s)=>t+s.valence,0)/e.length}function He(e){let t=e>=.5?"Very pleasant":e>=.15?"Pleasant":e>-.15?"Neutral":e>-.5?"Unpleasant":"Very unpleasant";return`<span style="color: hsl(${Math.round((e+1)/2*140)} 65% 42%); font-weight: 600;">${t}</span>`}var no=["Very Unpleasant","Unpleasant","Slightly Unpleasant","Neutral","Slightly Pleasant","Pleasant","Very Pleasant"];function us(){let e=new Date,t=s=>String(s).padStart(2,"0");return`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}T${t(e.getHours())}:${t(e.getMinutes())}`}function ps(e){let t=e?new Date(e).getTime():NaN;return isFinite(t)?t:Date.now()}function rs(e,t=[]){return e.map(s=>`<button type="button" class="chip${t.includes(s)?" active":""}" data-chip="${M(s)}">${M(s)}</button>`).join("")}function Re(e,t,s={}){for(let o of e.querySelectorAll(`${t} .chip`))o.addEventListener("click",()=>{s.single&&e.querySelectorAll(`${t} .chip`).forEach(n=>n.classList.remove("active")),o.classList.toggle("active",s.single?!0:!o.classList.contains("active"))})}var Fe=(e,t)=>[...e.querySelectorAll(`${t} .chip.active`)].map(s=>s.dataset.chip);function io(e){let t=W({html:`
      <div class="sheet-header">
      ...
      <div class="sheet-content">
        ...
        <div class="form-section">
          <div class="form-row"><input type="datetime-local" id="som-date" value="${us()}" style="text-align: left;" /></div>
        </div>
        <div style="height: 16px;"></div>
      </div>
    `,onMount(s){let o=s.querySelector("#som-val"),n=s.querySelector("#som-val-label"),i=()=>{let r=Number(o.value)+3,a=Number(o.value)/3,l=Math.round((a+1)/2*140);n.innerHTML=`<span style="color: hsl(${l} 65% 42%)">${no[r]}</span>`};i(),o.addEventListener("input",i),Re(s,"#som-kind",{single:!0}),Re(s,"#som-emotions"),Re(s,"#som-assoc"),s.querySelector("#som-cancel").addEventListener("click",()=>t()),s.querySelector("#som-save").addEventListener("click",async()=>{await ts({kind:Fe(s,"#som-kind")[0]||"momentaryEmotion",valence:Number(o.value)/3,labels:Fe(s,"#som-emotions"),associations:Fe(s,"#som-assoc"),date:ps(s.querySelector("#som-date").value)}),t(),I("Logged State of Mind"),e?.()})}})}function ro(e){let t=W({html:`
      <div class="sheet-header">
        ...
        <div class="form-section">
          <div class="form-row">
            <select id="dose-med" style="text-align: left;">
              ${n.map(r=>`<option value="${M(r.id)}"${r.id===s?" selected":""}>${M(r.nickname||r.concept.displayText)}</option>`).join("")}
            </select>
          </div>
        </div>
        <div class="section">Status</div>
        <div class="chip-row" id="dose-status">
          ${it.map(([r,a],l)=>`<button type="button" class="chip${l===0?" active":""}" data-chip="${r}">${M(a)}</button>`).join("")}
        </div>
        <div class="section">When</div>
        <div class="form-section">
          <div class="form-row"><input type="datetime-local" id="dose-date" value="${us()}" style="text-align: left;" /></div>
        </div>
      </div>
    `,onMount(r){Re(r,"#dose-status",{single:!0}),r.querySelector("#dose-cancel").addEventListener("click",()=>i()),r.querySelector("#dose-save").addEventListener("click",async()=>{await rt({medicationId:r.querySelector("#dose-med").value,status:Fe(r,"#dose-status")[0]||"taken",date:ps(r.querySelector("#dose-date").value),doseQuantity:1}),i(),I("Dose logged"),t?.()})}})}function hs(e){let t=!0,s=null;return e.container.innerHTML="",re().then(o=>{t&&(o?s=po(e,o):co(e))}).catch(o=>{t&&(e.container.innerHTML=`<div class="empty-state"><div class="empty-icon">!</div><h2>Couldn't load</h2><p>${M(o.message)}</p></div>`)}),()=>{t=!1,typeof s=="function"&&s()}}async function co(e){e.setTitle("Workout");let t=await X(),s=t[0],o=Qe(t),n=o?Ce(o.normalized):N[0],r=o&&fs(o.startedAt)==="today"?"Tomorrow":"Today",a=s?`<div class="last-workout-hint">Last: <strong>${M(s.name)}</strong> \xB7 ${fs(s.startedAt)}</div>`:"",l=`<div class="next-workout-hint">${r}: <strong>${M(n)}</strong></div>`;e.container.innerHTML=`
    ...
    <div class="action-section">
      ...
      <button id="nav-mind" class="btn-secondary">\u{1F9E0}&nbsp; State of Mind</button>
      <button id="nav-meds" class="btn-secondary">\u{1F48A}&nbsp; Medications</button>
    </div>
  `,e.container.querySelector("#start-btn").addEventListener("click",()=>lo(n,r)),e.container.querySelector("#nav-mind").addEventListener("click",()=>ct(e,()=>e.refresh())),e.container.querySelector("#nav-meds").addEventListener("click",()=>lt(e,()=>e.refresh()))}function fs(e){let t=new Date,s=new Date(e),o=i=>new Date(i.getFullYear(),i.getMonth(),i.getDate()).getTime(),n=Math.round((o(t)-o(s))/(1440*60*1e3));return n===0?"today":n===1?"yesterday":n<7?`${n} days ago`:n<14?"a week ago":`${Math.round(n/7)} weeks ago`}function lo(e,t="Today"){uo(e,async s=>{let o={id:F(),name:s,startedAt:Date.now(),endedAt:null,notes:""};await q("workouts",o),H("workout:changed")},t)}function uo(e,t,s="Today"){let n=W({html:`
      <div class="sheet-header">
      ...
      <div class="sheet-content">
        ...
        <div class="form-section">
          ${N.map(i=>{let a=i===e?` <span class="badge">${M(s)}</span>`:"";return`
              <button class="list-row button" data-name="${M(i)}">
                <div class="row-main"><div class="row-title" style="color: ${Ae(i)}; font-weight: 600;">${M(i)}${a}</div></div>
              </button>
            `}).join("")}
        </div>
  ...
  `,e.container.querySelector("#calc-fab").addEventListener("click",ko);let a=()=>{e.setTitle(kt((Date.now()-t.startedAt)/1e3))};a(),r=setInterval(a,1e3);let l=e.container.querySelector("#wname");l.addEventListener("input",async()=>{t.name=l.value,await q("workouts",{...t}),oe()});let u=new Map;e.container.querySelector("#add-exercise-btn").addEventListener("click",()=>{wo(s,i,async v=>{await ho(t,o,v),await b()})}),e.container.querySelector("#finish-btn").addEventListener("click",async()=>{if(confirm("Finish this workout?")){await go(t,o);try{let{filename:v}=await st();I(`Saved \xB7 backup: ${v}`)}catch(v){I(`Saved \xB7 backup failed: ${v.message}`)}H("workout:changed")}}),e.container.querySelector("#discard-btn").addEventListener("click",async()=>{confirm("Discard this workout? This cannot be undone.")&&(await Ee(t.id),H("workout:changed"))});async function b(){let[v,m,$]=await Promise.all([B("sets"),B("workouts"),B("exercises")]);s=$,o=v.filter(k=>k.workoutId===t.id).sort((k,x)=>k.order-x.order),n=ht(v,m,t.id),u=f(v,$,t.id),i=new Map;for(let k of v)i.set(k.exerciseId,(i.get(k.exerciseId)??0)+1);E(),S()}function S(){let v=new Map(s.map(g=>[g.id,g])),m=[],$=new Map;for(let g of o){let D=v.get(g.exerciseId);if(!D)continue;let L=R(D);if(m.includes(L)||m.push(L),!g.completed)continue;let C=(g.weight||0)*(g.reps||0);C<=0||$.set(L,($.get(L)??0)+C)}let k=[...$.values()].reduce((g,D)=>g+D,0),x=e.container.querySelector("#workout-progress");if(!x)return;if(m.length===0){x.innerHTML="";return}let T=m.map(g=>{let D=u.get(g)??0,L=$.get(g)??0;return{muscle:g,record:D,cur:L,span:Math.max(D,L)}}),A=Math.max(...T.map(g=>g.span)),p=A>0?A*.12:1;T=T.map(g=>({...g,span:Math.max(g.span,p)}));let c=Math.max(...T.map(g=>g.span)),h=T.map(({muscle:g,record:D,cur:L,span:C})=>{let P=C/c*100,V=L>0?Math.min(100,L/C*100):0,O;if(D>0){let ce=Math.round(L/D*100);O=L>D?`${ce}% \u{1F525}`:`${ce}%`}else O=L>0?"new \u{1F525}":"new";let U=D>0?`${Y(L)} / ${Y(D)} \xB7 ${O}`:`${Y(L)} \xB7 ${O}`,_=Dt(g);return`
        <div class="vol-muscle" style="width: ${P.toFixed(2)}%; --mcolor: ${_}; --mtext: ${Tt(_)};" title="${M(g)}: ${Y(L)} / record ${Y(D)} lbs">
    ...
    `,requestAnimationFrame(()=>{for(let g of x.querySelectorAll(".vol-muscle"))d(g)})}function d(v){let m=v.querySelector(".seg-name"),$=v.querySelector(".seg-vol"),k=v.clientWidth-4;if(k<=0)return;if($){let T=10;for($.style.fontSize=`${T}px`;$.scrollWidth>k&&T>6;)T-=.5,$.style.fontSize=`${T}px`}if(!m)return;m.style.display="";let x=11;for(m.style.fontSize=`${x}px`;m.scrollWidth>k&&x>5;)x-=.5,m.style.fontSize=`${x}px`}function f(v,m,$){let k=new Map(m.map(A=>[A.id,A])),x=new Map,T=new Map;for(let A of Q(v)){if(A.workoutId===$)continue;let p=k.get(A.exerciseId);if(!p)continue;let c=(A.weight||0)*(A.reps||0);if(c<=0)continue;let h=R(p),y=T.get(A.workoutId);y||T.set(A.workoutId,y=new Map),y.set(h,(y.get(h)??0)+c)}for(let A of T.values())for(let[p,c]of A)c>(x.get(p)??0)&&x.set(p,c);return x}async function w(v){if(!v.completed||(v.setType||"working")==="warmup"||!(v.weight>0)||!(v.reps>0))return;let m=s.find(c=>c.id===v.exerciseId);if(!m)return;let $=await B("sets"),k=Q($).filter(c=>c.exerciseId===v.exerciseId&&c.id!==v.id&&(c.setType||"working")!=="warmup"&&c.weight>0&&c.reps>0);if(k.length===0)return;let x=[],T=k.reduce((c,h)=>Math.max(c,h.weight),0);v.weight>T&&x.push(`Heaviest weight ever: ${me(v.weight)} lbs`);let A=v.weight*v.reps,p=k.reduce((c,h)=>Math.max(c,h.weight*h.reps),0);if(A>p&&x.push(`Most volume in a set: ${me(v.weight)}\xD7${v.reps} = ${Y(A)} lbs`),x.length>0){let c=x.length>1?"New records":"New record";I(`\u{1F3C6} ${K(m)} \u2014 ${c}!
${x.join(`
`)}`,0,{persistUntilClick:!0})}}function E(){let v=new Map(s.map(p=>[p.id,p])),m=[],$=new Map;for(let p of o)$.has(p.exerciseId)||($.set(p.exerciseId,[]),m.push(p.exerciseId)),$.get(p.exerciseId).push(p);for(let[,p]of $)p.sort((c,h)=>c.order-h.order);let k=e.container.querySelector("#exercise-sections");if(m.length===0){k.innerHTML=`
        <div class="empty-state" style="padding: 40px 24px; min-height: auto;">
          <p style="color: var(--text-secondary);">Add an exercise to start logging sets.</p>
        </div>`;return}k.innerHTML=m.map(p=>{let c=v.get(p),h=$.get(p),y=n.get(p)??new Map;return fo(c,h,y,i.get(p)??0)}).join("");function x(p){delete p.bumpedBy,delete p.preBumpWeight,delete p.preBumpReps}function T(p){let c=o.filter(L=>L.exerciseId===p.exerciseId).sort((L,C)=>L.order-C.order),h=p.setType||"working",y=0,g=0;for(let L of c)if(g+=1,(L.setType||"working")===h&&(y+=1),L.id===p.id)break;let D=we(h,y,n.get(p.exerciseId),g);return D&&D.weight>0&&D.reps>0?{weight:D.weight,reps:D.reps}:null}async function A(p){await vs(p.id,o),p.completed&&await ms(p,o,T);for(let c of o){if(c.exerciseId!==p.exerciseId)continue;let h=k.querySelector(`.set-row[data-set-id="${c.id}"]`);if(!h)continue;let y=h.querySelector(".weight-input"),g=h.querySelector(".reps-input");y&&document.activeElement!==y&&(y.value=c.weight>0?String(c.weight):""),g&&document.activeElement!==g&&(g.value=c.reps>0?String(c.reps):"")}}for(let p of k.querySelectorAll(".set-row-wrap")){let c=p.querySelector(".set-row"),h=c.dataset.setId,y=o.find(O=>O.id===h);if(!y)continue;let g=c.querySelector(".weight-input"),D=c.querySelector(".reps-input"),L=c.querySelector(".complete-btn");vo(p,async()=>{await ie("sets",y.id),await b()});let C=Ye(async()=>{await A(y),y.completed&&S()},200);g.addEventListener("input",()=>{y.weight=parseFloat(g.value)||0,x(y),q("sets",{...y}).catch(O=>console.error("Set save failed",O)),C()});let P=Ye(async()=>{await A(y),y.completed&&S()},200);D.addEventListener("input",()=>{y.reps=parseInt(D.value,10)||0,x(y),q("sets",{...y}).catch(O=>console.error("Set save failed",O)),P()}),L.addEventListener("click",async()=>{let O=y.completed;y.completed=!y.completed,y.completed&&x(y),await q("sets",y),c.classList.toggle("completed",y.completed),L.innerHTML=ys(y.completed);let U=c.querySelector(".set-number")?.textContent?.trim()||"";L.setAttribute("aria-label",`${y.completed?"Mark incomplete":"Mark complete"} set ${U}`),S(),!O&&y.completed?(await ms(y,o,T)&&E(),await w(y)):O&&!y.completed&&await vs(y.id,o)&&E()});let V=c.querySelector(".set-number");V&&V.addEventListener("click",async()=>{let U=(y.setType||"working")==="warmup"?"working":"warmup";if(y.setType=U,!y.completed){let _=o.filter(ne=>ne.exerciseId===y.exerciseId).sort((ne,bs)=>ne.order-bs.order),ce=0,ut=0;for(let ne of _)if(ut+=1,(ne.setType||"working")===U&&(ce+=1),ne.id===y.id)break;let le=we(U,ce,n.get(y.exerciseId),ut);le&&le.weight>0&&le.reps>0&&(y.weight=le.weight,y.reps=le.reps)}await q("sets",y),E()})}for(let p of k.querySelectorAll(".add-set-btn"))p.addEventListener("click",async()=>{let c=p.dataset.exerciseId;await yo(t,o,c,n.get(c)??new Map),await b()});for(let p of k.querySelectorAll(".exercise-menu"))p.addEventListener("click",async()=>{let c=p.dataset.exerciseId;confirm("Remove this exercise from the workout?")&&(await Ve("sets",o.filter(h=>h.exerciseId===c).map(h=>h.id)),await b())});for(let p of k.querySelectorAll(".exercise-name-btn"))p.addEventListener("click",()=>{r&&(clearInterval(r),r=null),We(e,p.dataset.exerciseId,()=>e.refresh())})}return b(),()=>{r&&clearInterval(r)}}function fo(e,t,s=new Map,o=0){let n=0,i=0,r=t.map((a,l)=>{let u=a.setType||"working",b,S;u==="warmup"?(i+=1,S=i,b=`W${i}`):(n+=1,S=n,b=String(n));let d=we(u,S,s,l+1);return mo(a,b,d)}).join("");return`
    <div class="exercise-section">
      <div class="exercise-section-header">
        <button class="exercise-name-btn" data-exercise-id="${e?.id}">${te(e)}</button>
        <div class="row-trailing trailing-stack">${se(o)}</div>
        <button class="menu exercise-menu" data-exercise-id="${e?.id}" aria-label="Remove ${M(K(e))} from workout">\xD7</button>
      </div>
  ...
  `}function we(e,t,s,o=null){if(!s||typeof s.get!="function")return null;let n=s.get(`${e}#${t}`);return n||(o!=null?s.get(`any#${o}`)??null:null)}function mo(e,t,s){let o=e.setType||"working",n=s&&s.weight>0&&s.reps>0?`${me(s.weight)} \xD7 ${s.reps}`:"\u2014";return`
  ...
  `}function vo(e,t){let s=e.querySelector(".set-row"),o=e.querySelector(".set-swipe-delete");if(!s||!o)return;let n=88,i=0,r=0,a=0,l=0,u=!1,b=!1,S=!1,d=!1,f=()=>Math.max(140,i*.5);function w(k,x){s.style.transition=x?"transform 0.18s ease":"none",s.style.transform=`translateX(${k}px)`,o.style.width=`${Math.max(n,-k)}px`,e.classList.toggle("will-delete",k<=-f())}function E(k=!0){S=!1,w(0,k),e.classList.remove("swiped-open")}function v(k=!0){document.querySelectorAll(".set-row-wrap.swiped-open").forEach(x=>{if(x!==e){let T=x.querySelector(".set-row");T&&(T.style.transition="transform 0.18s ease",T.style.transform="translateX(0)");let A=x.querySelector(".set-swipe-delete");A&&(A.style.width=""),x.classList.remove("swiped-open","will-delete")}}),S=!0,w(-n,k),e.classList.add("swiped-open")}function m(){s.style.transition="transform 0.16s ease-out",s.style.transform=`translateX(${-i}px)`,o.style.width=`${i}px`,setTimeout(t,150)}s.addEventListener("touchstart",k=>{i=e.clientWidth||s.clientWidth,r=k.touches[0].clientX,a=k.touches[0].clientY,l=S?-n:0,u=!0,b=!1,d=!!k.target.closest("input, button, select, textarea")},{passive:!0}),s.addEventListener("touchmove",k=>{if(!u)return;let x=k.touches[0].clientX-r,T=k.touches[0].clientY-a;if(!b){if(Math.abs(T)>Math.abs(x)+4){u=!1;return}Math.abs(x)>8&&(b=!0,d&&document.activeElement?.blur&&document.activeElement.blur())}if(!b)return;k.cancelable&&k.preventDefault();let A=S?-n:0;l=Math.min(0,Math.max(-i,A+x)),w(l,!1)},{passive:!1});function $(){u&&(u=!1,b&&(l<=-f()?m():l<-n/2?v():E()))}s.addEventListener("touchend",$),s.addEventListener("touchcancel",$),o.addEventListener("click",k=>{k.stopPropagation(),t()})}function ys(e){return e?'<svg viewBox="0 0 24 24" width="26" height="26" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>':'<svg viewBox="0 0 24 24" width="26" height="26" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="9"/></svg>'}async function ho(e,t,s){let o=t.reduce((n,i)=>Math.max(n,i.order),-1)+1;for(let n of s){let i=(await vt(n,e.id)).filter(l=>(l.weight||0)>0&&(l.reps||0)>0),a=(i.length>0?i:[{weight:0,reps:0,setType:"working"}]).map(l=>({id:F(),workoutId:e.id,exerciseId:n,weight:l.weight??0,reps:l.reps??0,setType:l.setType||"working",completed:!1,order:o++,createdAt:Date.now()}));await fe("sets",a)}}async function ms(e,t,s){let o=(e.weight||0)*(e.reps||0);if(o<=0)return!1;let n=!1;for(let i of t)if(i.exerciseId===e.exerciseId&&i.id!==e.id&&!((i.order??0)<=(e.order??0))&&!i.completed&&(i.weight||0)*(i.reps||0)<o){if(i.bumpedBy==null){let r=s?.(i);i.preBumpWeight=r?r.weight:i.weight,i.preBumpReps=r?r.reps:i.reps}i.bumpedBy=e.id,i.weight=e.weight,i.reps=e.reps,await q("sets",i),n=!0}return n}async function vs(e,t){let s=!1;for(let o of t)o.bumpedBy===e&&(o.completed||(o.preBumpWeight!=null&&(o.weight=o.preBumpWeight),o.preBumpReps!=null&&(o.reps=o.preBumpReps)),delete o.bumpedBy,delete o.preBumpWeight,delete o.preBumpReps,await q("sets",o),s=!0);return s}async function yo(e,t,s,o=new Map){let n=t.filter(E=>E.exerciseId===s),i=n[n.length-1],r=E=>(E?.weight||0)*(E?.reps||0),a=n.filter(E=>(E.setType||"working")!=="warmup"),l=a.length+1,u=we("working",l,o,n.length+1),b=a.filter(E=>E.weight>0&&E.reps>0).reduce((E,v)=>!E||r(v)>r(E)?v:E,null),S=a.some((E,v)=>{let m=we("working",v+1,o);return m&&m.weight>0&&m.reps>0&&r(E)>r(m)}),d=i?.weight??0,f=i?.reps??0;b&&(!u||S)&&(d=b.weight,f=b.reps);let w={id:F(),workoutId:e.id,exerciseId:s,weight:d,reps:f,completed:!1,order:(i?.order??-1)+1,createdAt:Date.now()};await q("sets",w)}async function go(e,t){await Ve("sets",t.filter(s=>!s.completed&&((s.weight||0)===0||(s.reps||0)===0)).map(s=>s.id)),e.endedAt=Date.now(),await q("workouts",e)}function wo(e,t,s){let o=new Set,n="",i=null,r=W({html:`
      <div class="sheet-header">
    ...
    `,onMount(a){let l=a.querySelector("#picker-list"),u=a.querySelector("#picker-add"),b=a.querySelector("#picker-cancel"),S=a.querySelector("#picker-custom"),d=a.querySelector("#picker-search"),f=a.querySelector("#picker-chips");function w(){f.innerHTML=Be(e,i);for(let v of f.querySelectorAll(".chip"))v.addEventListener("click",()=>{let m=v.dataset.cat;i=m==="All"?null:m,w(),E()})}function E(){let v=e.filter(m=>!i||R(m)===i).filter(m=>!n||m.name.toLowerCase().includes(n.toLowerCase())).sort((m,$)=>{let k=t.get(m.id)??0,x=t.get($.id)??0;return k!==x?x-k:m.name.localeCompare($.name)});l.innerHTML=v.length===0?'<div class="list-row"><div class="row-main" style="color:var(--text-secondary)">No matches</div></div>':v.map(m=>`
              ...
              `).join("");for(let m of l.querySelectorAll(".list-row[data-id]"))m.addEventListener("click",()=>{let $=m.dataset.id;o.has($)?o.delete($):o.add($),u.disabled=o.size===0,u.textContent=o.size===0?"Add":`Add (${o.size})`,E()})}d.addEventListener("input",()=>{n=d.value,E()}),b.addEventListener("click",()=>r()),u.addEventListener("click",()=>{s(Array.from(o)),r()}),S.addEventListener("click",()=>{ge(null,async v=>{e.push(v),o.add(v.id),w(),E(),u.disabled=!1,u.textContent=`Add (${o.size})`})}),w(),E()}})}function bo(){return'<svg viewBox="0 0 24 24" width="22" height="22" fill="var(--green)"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>'}function ge(e,t){let s=!!e,o=s?R(e):null,n=!o||he.includes(o)?he:[o,...he],i=e?.equipment,r=!i||De.includes(i)?De:[i,...De],a=W({html:`
      <div class="sheet-header">
      ...
      <div class="sheet-content">
        ...
        <div class="form-section">
          <div class="form-row">
            ...
            <select id="ce-cat">${n.map(l=>`<option${l===o?" selected":""}>${M(l)}</option>`).join("")}</select>
          </div>
        </div>
        ...
        <div class="form-section">
          <div class="form-row">
            ...
            <select id="ce-eq">${r.map(l=>`<option${l===i?" selected":""}>${M(l)}</option>`).join("")}</select>
          </div>
        </div>
      </div>
    `,onMount(l){let u=l.querySelector("#ce-name"),b=l.querySelector("#ce-save");u.addEventListener("input",()=>{b.disabled=u.value.trim().length===0}),l.querySelector("#ce-cancel").addEventListener("click",()=>a()),b.addEventListener("click",async()=>{let S=u.value.trim();if(!S)return;let d=l.querySelector("#ce-cat").value,f=l.querySelector("#ce-eq").value,w=s?{...e,name:S,muscle:d,equipment:f}:{id:F(),name:S,muscle:d,category:d,equipment:f,notes:"",isCustom:!0,createdAt:Date.now()};await q("exercises",w),a(),t?.(w),s||H("data:changed")}),s||setTimeout(()=>u.focus(),50)}})}function ko(){let t=[["AC","clear","fn"],["\xB1","sign","fn"],["\u232B","back","fn"],["\xF7","op","op"],["7","digit"],["8","digit"],["9","digit"],["\xD7","op","op"],["4","digit"],["5","digit"],["6","digit"],["\u2212","op","op"],["1","digit"],["2","digit"],["3","digit"],["+","op","op"],["0","digit","zero"],[".","dot"],["=","equals","op"]].map(([s,o,n])=>`<button class="calc-key${n?` calc-${n}`:""}" data-action="${o}" data-key="${M(s)}">${M(s)}</button>`).join("");W({html:`
      <div class="sheet-header">
        <span style="width: 60px;"></span>
    ...
    `,onMount(s,o){let n=s.querySelector("#calc-expr"),i=s.querySelector("#calc-result"),r={"+":(c,h)=>c+h,"\u2212":(c,h)=>c-h,"\xD7":(c,h)=>c*h,"\xF7":(c,h)=>h===0?NaN:c/h},a=c=>c==="+"||c==="\u2212"||c==="\xD7"||c==="\xF7",l=c=>{if(!isFinite(c))return"Error";let h=parseFloat(c.toFixed(8)).toString();return h.replace("-","").replace(".","").length>12&&(h=c.toPrecision(10).replace(/\.?0+$/,"")),h},u=["0"],b=!1,S=!1,d="",f=()=>u[u.length-1];function w(){n.textContent=S?"":d,i.textContent=S?"Error":u.join(" ");let c=!S&&a(f())?f():null;for(let h of s.querySelectorAll(".calc-op"))h.classList.toggle("selected",h.dataset.key===c)}function E(c){if(S&&(u=["0"],S=!1),b)return u=[c],b=!1,w();a(f())?u.push(c):u[u.length-1]=f()==="0"?c:f()+c,w()}function v(){if(S&&(u=["0"],S=!1),b)return u=["0."],b=!1,w();a(f())?u.push("0."):f().includes(".")||(u[u.length-1]=f()+"."),w()}function m(c){S||(b=!1,a(f())?u[u.length-1]=c:u.push(c),w())}function $(){u=["0"],b=!1,S=!1,w()}function k(){if(S||a(f()))return;let c=f();u[u.length-1]=c.startsWith("-")?c.slice(1):c==="0"?"0":"-"+c,w()}function x(){if(S)return $();if(b=!1,a(f()))return u.pop(),w();let c=f().slice(0,-1);c===""||c==="-"?u.length>1?u.pop():u=["0"]:u[u.length-1]=c,w()}function T(){if(S)return;let c=u.slice();if(a(c[c.length-1])&&c.pop(),c.length<3)return;let h=parseFloat(c[0]);for(let y=1;y<c.length;y+=2)if(h=r[c[y]](h,parseFloat(c[y+1])),!isFinite(h))return S=!0,w();d=`${c.join(" ")} =`,u=[l(h)],b=!0,w()}function A(c){let{action:h,key:y}=c.dataset;h!=="equals"&&(d=""),h==="digit"?E(y):h==="dot"?v():h==="clear"?$():h==="sign"?k():h==="back"?x():h==="op"?m(y):h==="equals"&&T()}let p=null;for(let c of s.querySelectorAll(".calc-key"))c.addEventListener("pointerdown",h=>{h.preventDefault(),p=c,c.classList.add("pressed")}),c.addEventListener("pointerup",h=>{h.preventDefault(),c.classList.remove("pressed"),p===c&&A(c),p=null}),c.addEventListener("pointercancel",()=>{c.classList.remove("pressed"),p=null}),c.addEventListener("pointerleave",()=>c.classList.remove("pressed"));s.querySelector("#calc-done").addEventListener("click",()=>o())}})}function xe(){let e=window.matchMedia("(display-mode: standalone)").matches||window.navigator.standalone===!0,t;e?t=Math.max(window.innerHeight||0,window.visualViewport?.height||0,window.screen?.height||0):t=window.visualViewport?.height||window.innerHeight,document.documentElement.style.setProperty("--app-height",`${t}px`)}xe();window.addEventListener("resize",xe);window.addEventListener("orientationchange",xe);window.addEventListener("pageshow",xe);window.visualViewport?.addEventListener("resize",xe);var gs={workout:{title:"Workout",render:hs},exercises:{title:"Exercises",render:Yt},progress:{title:"Progress",render:Nt}},be=document.getElementById("view-content"),xo=document.getElementById("nav-title"),ws=document.getElementById("nav-back"),G=document.getElementById("nav-action"),ke="workout",dt=null,ze=null,je=null,Ne={container:be,setTitle(e){xo.textContent=e},setAction(e){if(!e){G.hidden=!0,G.innerHTML="",G.removeAttribute("aria-label"),ze=null;return}G.hidden=!1,e.label?G.setAttribute("aria-label",e.label):G.removeAttribute("aria-label"),e.html?G.innerHTML=e.html:G.textContent=e.label??"",ze=e.onClick},setBack(e){dt=e,ws.hidden=!e},refresh(){Se(ke)},toast(e){I(e)}};function So(){if(typeof je=="function")try{je()}catch(e){console.error(e)}je=null}function Se(e){ke=e,Lt(e),document.querySelectorAll(".tab").forEach(t=>{t.setAttribute("aria-selected",String(t.dataset.tab===e))}),So(),Ne.setTitle(gs[e].title),Ne.setAction(null),Ne.setBack(null),be.innerHTML="",be.scrollTop=0;try{je=gs[e].render(Ne)}catch(t){console.error("Render failed",t),be.innerHTML=`<div class="empty-state"><div class="empty-icon">!</div><h2>Render error</h2><p>${M(t.message)}</p></div>`}}document.querySelectorAll(".tab").forEach(e=>{e.addEventListener("click",()=>{document.querySelectorAll(".sheet-backdrop").forEach(t=>t.dismissSheet?.()),Se(e.dataset.tab)})});ws.addEventListener("click",()=>{dt&&dt()});G.addEventListener("click",()=>{ze&&ze()});Ke("data:changed",()=>{oe(),Se(ke)});Ke("workout:changed",()=>{oe(),ke==="workout"&&Se(ke)});document.addEventListener("visibilitychange",()=>{document.visibilityState==="visible"&&oe()});async function $o(){try{await j();let e=await Mt();e>0&&console.info(`Seeded ${e} exercises.`),await It(),Se("workout"),oe()}catch(e){console.error("Init failed:",e),be.innerHTML=`
      <div class="empty-state">
