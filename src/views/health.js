...
import {
  loadHealth, moodVsWorkouts,
  saveStateOfMind, saveMedication, deleteHealthRecord,
  EMOTION_LABELS, ASSOCIATION_LABELS,
} from '../health.js';
...
export async function renderMedicationsPage(ctx, onBack) {
  ...
  const { medications } = await loadHealth();

  ctx.container.innerHTML = medications.length ? `
    <div class="section">Daily</div>
    ${medList(medications.filter((m) => m.hasSchedule))}
    ${medList(medications.filter((m) => !m.hasSchedule), 'As needed')}
    <div class="section-footer">Tap a medication to edit its name, form, or amount.</div>
  ` : emptyState('\u{1F48A}', 'No medications', 'Tap \uFF0B to add the medications you take.');
  ctx.container.scrollTop = 0;
...
/** A list of medications under an optional heading; nothing renders when the
 * group is empty (so someone with no as-needed meds sees no stray section). */
function medList(meds, heading) {
  if (meds.length === 0) return '';
  return `
    ${heading ? `<div class="section">${esc(heading)}</div>` : ''}
    <div class="list">${meds.map(medRow).join('')}</div>`;
}

function medRow(m) {
  const sub = medSubtitle(m);
  return `
    <button class="list-row" data-edit-med="${esc(m.id)}">
      <div class="row-main">
        <div class="row-title">${esc(m.nickname || m.concept.displayText)}</div>
...
/**
 * What you take, in one line \u2014 "4 \u00D7 1.25 g capsule", "10 mg tablet",
 * "2 puffs \u00B7 90 mcg inhaler". The unit and the form often name the same
 * thing, so combine them instead of printing both ("1 tablet \u00B7 10 mg tablet").
 */
function medSubtitle(m) {
  const amount = doseAmountOf(m);
  const form = (m.concept?.form || '').trim();
  const unit = (m.doseUnit || '').trim();
  if (form && amount > 1 && unit && form.toLowerCase().includes(unit.toLowerCase())) {
    return `${formatQty(amount)} \u00D7 ${form}`;
  }
  if (form && amount === 1) return form;
  const qty = doseLabel(amount, unit);
  return form ? `${qty} \u00B7 ${form}` : qty;
}

/** Amount taken per dose, defaulting to 1 for entries saved before the field. */
const doseAmountOf = (m) => (Number(m?.doseAmount) > 0 ? Number(m.doseAmount) : 1);

/** "4 capsules", "1 tablet", "5 mg" \u2014 measurement units (mg/ml/g\u2026) don't
 * pluralize; countable ones (capsule, tablet, puff) do. */
function doseLabel(qty, unit) {
  const u = (unit || '').trim() || 'dose';
  const plural = qty === 1 || /^(mg|mcg|ml|cc|g|kg|l|oz|iu)$/i.test(u) || u.endsWith('s') ? u : `${u}s`;
